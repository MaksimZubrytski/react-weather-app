import { getBackgroundImage } from "../../api/backgroundApi/provider";
import { getCityCoordinates } from "../../api/searchApi/provider";
import { getUserCoordinatesData } from "../../api/userCoordinateApi/provider";
import { getWeatherForecast } from "../../api/weatherApi/provider";
import { setBackground } from "../actions/background-actions";
import { setCoordinatesData, setSearchData, toggleIsFetching, setError }
...
export const initAppState = (lang) => (
  (dispatch) => {
    ...
    getBackgroundImage().then((data) => {
      ...
      getUserCoordinatesData().then(({ userCity, userLocation }) => {
...
export const initSearchState = (city, lang) => (
  (dispatch) => {
    dispatch(setError(false));
    dispatch(toggleIsFetching(true));
    getCityCoordinates(city).then(({ place, lat, lng }) => {
      if (place === undefined) {
